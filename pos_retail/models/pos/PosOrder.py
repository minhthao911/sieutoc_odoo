...
class POSOrder(models.Model):
    ...
    state = fields.Selection(selection_add=[
        ('quotation', 'Waiting Payment (Quotation)')
    ], ondelete={
    ...
    tracking_ids = fields.One2many('pos.tracking.order', 'order_id', 'Tracking Logs')
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        if context.get('pos_config_id', None):
            ...
            if config.load_orders_limit > 0:
                limit = config.load_orders_limit
            else:
                limit = 100
            order = 'id DESC'
    ...
    def write(self, vals):
        ...
        for order in self:
            ...
            if vals.get('state', None):
                OrderJson = self.env['pos.order.json'].search([('order_id', '=', order.id)], limit=1)
                if OrderJson:
                    OrderJson.write({
                        'name': order.name,
                        'state': order.state
    ...
    def auto_closing_backup_session(self):
        if self.session_id and self.session_id.backup_session:
            ...
            if not orders_not_paid:
                self.session_id.close_session_without_popup()
        return True
    ...
    def create(self, vals):
        ...
        OrderJson = self.env['pos.order.json'].search([('ean13', '=', order.ean13)], limit=1)
        if OrderJson:
            OrderJson.write({
                'order_id': order.id,
                'state': order.state
        ...
        # for user in users:
        #     self.env['bus.bus']._sendone(user.partner_id, 'pos.new.order', {
...
class POSOrderLine(models.Model):
    ...
    price_change = fields.Boolean('Price Change')
