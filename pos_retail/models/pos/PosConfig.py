...
class PosConfig(models.Model):
    ...
    def init(self):
        self.env.cr.execute(
            """DELETE FROM ir_model_data WHERE model IN ('pos.bus', 'pos.bus.log')""");
    ...
    kitchen_receipt = fields.Boolean(
        ...
        help='If have products not submit to Kitchen \n'
             'When print Receipt all products will display on Receipt'
    )
    ...
    cache = fields.Selection([
        ('none', 'Direct Odoo Server'),
        ('browse', 'Web Browse'),
    ],
        default='none',
        string='Loading Products and Customer type',
        help='Noted: POS Config and POS Session never cache, it will refresh with POS Screen refresh \n'
             '1) Direct Odoo Server: Loading all pos datas direct Odoo Server \n'
             '2) Web Browse: All pos datas will Cached to Users Browse \n'
    )
    ...
    employee_automatic_login = fields.Boolean('Automatic Login with Employee linked to User')
    number_of_orders_sessions_opened = fields.Integer(
        string="Number of Orders Session Opened",
        compute='_compute_current_session'
    )
    total_payments_amount = fields.Float(
        compute='_compute_current_session',
        string='Total Payments Amount of Sessions Opened'
    )
    disable_close_session = fields.Boolean(
        'Disable Close Session of POS Screen',
        help='When pos user click to Logout button on right top header POS Screen \n'
             'Will have 1 popup and have one button Close Session on footer of popup \n'
             'If this field is checked, this Button Close Session will hide'
    )

    @api.depends('session_ids', 'session_ids.state')
    def _compute_current_session(self):
        super(PosConfig, self)._compute_current_session()
        for pos_config in self:
            pos_config.number_of_orders_sessions_opened = 0
            pos_config.total_payments_amount = 0.00
            opened_sessions = pos_config.session_ids.filtered(lambda s: s.state != 'closed')
            for s in opened_sessions:
                pos_config.number_of_orders_sessions_opened += len(s.order_ids)
                pos_config.total_payments_amount += s.total_payments_amount

    # TODO: from pos menu kanban, if sessions have opened or opening_control, allow user start pos screen , remove step go to pos session form
    def open_existing_session_cb(self):
        res = super(PosConfig, self).open_existing_session_cb()
        if self.current_session_id.id and self.current_session_id.state in ['opening_control', 'opened']:
            return self.open_ui()
        else:
            return res
    ...
    def getOrdersNotFullFillPaymentAmount(self, config_id):
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        if context.get('action_pos_config_kanban', None) and not self.env.user.has_group(
                'point_of_sale.group_pos_manager') and not self.env.user.has_group('base.group_erp_manager'):
            domain = ['|', ('user_id', '=', self.env.user.id),
                      ('id', 'in', [c.id for c in self.env.user.pos_config_ids])]
        return super().search_read(domain=domain, fields=fields, offset=offset, limit=limit, order=order)

    def get_points_sync(self, base_config_id):
        config = self.browse(base_config_id)
        points = []
        if config.module_pos_hr:
            for employee in config.employee_ids:
                point = '%s_%s' % (config.id, employee.id)
                if point not in points:
                    points.append(point)
        for sync_config in config.sync_to_pos_config_ids:
            if sync_config.module_pos_hr:
                for employee in sync_config.employee_ids:
                    point = '%s_%s' % (sync_config.id, employee.id)
                    if point not in points:
                        points.append(point)
            else:
                point = sync_config.id
                if point not in points:
                    points.append(point)
        return points
