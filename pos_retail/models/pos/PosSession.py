...
class PosSession(models.Model):
        ...
        if vals.get('state', None):
            ...
            for session in sessions:
                self.env['bus.bus']._sendone(
                    session.user_id.partner_id, 'pos.update.sync.points', session.user_id.partner_id.id)
    ...
    def update_closing_control_state_session(self, notes=None):
        if notes == None:
            notes = _("Close Manual by user %s" % self.env.user.login)
        return super(PosSession, self).update_closing_control_state_session(notes=notes)
