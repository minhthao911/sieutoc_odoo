    ...
    models.Order = models.Order.extend({
        initialize: function (attributes, options) {
            ...
            if (!options.json) {
                ...
                this.checkin_time = field_utils.format.datetime(
                    moment(this.checkin_time), {}, {timezone: false});
            }
        ...
        export_as_JSON: function () {
            ...
            if (this.checkin_time) {
                json.checkin_time = this.checkin_time
            }
            return json;
        },
        ...
        export_for_printing: function () {
            ...
            if (this.pos.tables && this.pos.tables.length) {
                receipt['restaurant'] = true
                receipt['checkin_time'] = order.checkin_time
                receipt['checkout_time'] = new Date()
                receipt['checkout_time'] = field_utils.format.datetime(
                    moment(receipt['checkout_time']), {}, {timezone: false});
                if (order.table) {
                    receipt['table'] = order.table.name
                }
            }
            receipt['order'] = order
        ...
        generate_unique_id: function () {
            ...
            let uid_order = _super_Order.generate_unique_id.apply(this, arguments);
            uid_order = uid_order + '-' + zero_pad(this.pos.user.id, 5);
            return uid_order
        },
        is_paid: function () {
        ...
        remove_orderline: async function (line) {
            ...
            if (line.waitingApproveId) {
                rpc.query({
                    ...
                    args: [[], line.waitingApproveId],
                    context: {}
                })
            }
            return _super_Order.remove_orderline.apply(this, arguments);
        },
        ...
        checking_apply_price_filter_by_quantity_of_product: function (promotion) {
            ...
            if (!rules) {
                console.error(' rules of promotion not found, promotion id: ' + promotion.id)
                return false
            }
            let product_quantity_by_product_id = this.product_quantity_by_product_id();
            for (let i = 0; i < rules.length; i++) {
    ...
    models.Orderline = models.Orderline.extend({
        ...
        set_unit_price: async function (price) {
            ...
            if (this.pos.config.validate_price_change && currentPrice != this.product.get_price()) {
                ...
                const waitingApproveId = await this.order._requestApproveAction(action, description, action_strId, type, this.product.id)
