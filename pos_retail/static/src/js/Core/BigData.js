odoo.define('pos_retail.big_data', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        initialize: async function (session, attributes) {
            ...
            this.cache_type = this.session.config.cache
            ...
            pos_session_object.domain = function (self) {
                console.info('>>> force domain of pos session')
                var domain = [
        ...
        set_last_write_date_by_model: function (model, results) {
            ...
            try {
                for (let i = 0; i < results.length; i++) {
                    ...
                    if (this.write_date_by_model[model] != line.write_date && new Date(this.write_date_by_model[model]).getTime() < (new Date(line.write_date).getTime() + 1000)) {
                        this.write_date_by_model[model] = line.write_date
                    }
                }
                // TODO: Removed because have bug with multi language (removed date: 14.04.2022)
    ...
    models.load_models([
        {
            ...
            condition: function (self) {
                if (self.cache_type == 'browse' && self.total_products == 0) {
                    return true
                } else {
                    return false
                }
        ...
        },
        {
            ...
            condition: function (self) {
                if (self.cache_type == 'browse' && self.total_clients == 0) {
                    return true
                } else {
                    return false
                }
        ...
        },
        {
            ...
            domain: function (self) {
                ...
                if (self.config.pos_branch_id) {
                    domain.push('|')
                    domain.push(['pos_branch_id', '=', self.config.pos_branch_id[0]])
                    domain.push(['pos_branch_id', '=', null])
                }
                return domain
            },
