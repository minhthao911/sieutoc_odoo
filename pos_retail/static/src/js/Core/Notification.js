...
odoo.define('pos_retail.notification', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        async getOrdersNotFullFillPaymentAmount(reCall) {
            let self = this;
            console.log('getOrdersNotFullFillPaymentAmount')
            const orders = await rpc.query({
                model: 'pos.config',
                method: 'getOrdersNotFullFillPaymentAmount',
                args: [[], this.config.id]
            }, {
                shadow: true,
                timeout: 60000
                ...
                if (!reCall) {
                    setTimeout(_.bind(self.getOrdersNotFullFillPaymentAmount, self), self.config.notification_payment_time * 60 * 1000);
                }
            })
            if (orders) {
                const list = []
        ...
        startingNotification: function () {
            if (this.config.notification_payment_time > 0) {
                setTimeout(_.bind(this.getOrdersNotFullFillPaymentAmount, this), this.config.notification_payment_time * 60 * 1000);
            }
