odoo.define('pos_retail.synchronization', function (require) {
    ...
    exports.pos_bus = Backbone.Model.extend({
        ...
        async on_notification(notifications) {
            if (notifications && notifications[0] && notifications[0][1]) {
                ...
                if (type == 'pos.update.sync.points') {
                    console.warn('>>>> event pos.sync.session !!!')
                    console.log(type)
                    console.log(payload)
                    await this._build_send_to()
                }
            }
        },
        ...
        set_online: function () {
            ...
            self.pos.set('syncStatus', {status: 'connected', pending: 'Sync Sessions'});
        },
            ...
            async function waitSync() {
                ...
                for (let i = 0; i < self.pos.iot_connections.length; i++) {
                    ...
                    let params = {
                        ...
                        config_ids: self.sync_to_points || [],
                    };
        ...
        _build_send_from: function () {
            if (!this.pos.config.module_pos_hr) {
                ...
                return this.pos.config.id + '_' + this.pos.get_cashier().id
            }
        },
        ...
        async _build_send_to() {
            ...
            console.log('>>>>_build_send_to')
            console.log('points sync: ' + this.sync_to_points)
        },

        async send_notification(value, send_manual = false) {
            let self = this;
            ...
            let params = {
                ...
                config_ids: this.sync_to_points || [],
                message: value,
            };
            for (let i = 0; i < this.pos.iot_connections.length; i++) { //
                let iot_connection = this.pos.iot_connections[i];
    ...
    models.PosModel = models.PosModel.extend({
        ...
        _save_to_server: function (orders, options) {
            if (orders && orders.length != 0 && this.config.pos_order_tracking) {
                for (let i = 0; i < orders.length; i++) {
                    let order = orders[i]
                    order['data'].tracking_ids.push({
                        'name': order['data'].name,
                        'order_reference': order['data'].uid,
        ...
        async load_orders() {
            ...
            await _super_PosModel.load_orders.apply(this, arguments);
        ...
        sync_lock_table: function (data) {
            ...
            for (let floor_id in this.floors_by_id) {
                let tables = this.floors_by_id[floor_id]['tables']
                let table_will_lock = _.find(tables, function (tb) {
                    return tb.id == data.table_id
                })
                if (table_will_lock) {
        ...
        async sync_set_client(vals) {
            ...
            if (!client) {
                ...
                client = this.db.get_partner_by_id(partner_id)
                if (!client) {
        ...
        initialize: function (attributes, options) {
            ...
            if (this.pos.pos_bus) {
                ...
                this.orderlines.bind('add', function (line) {
                    ...
                    if (self.pos.config.pos_order_tracking) {
                        line.order.tracking_ids.push({
                            'name': _t('Add new product: ') + line.product.display_name,
                            'order_reference': line.order.uid,
                            'user_id': self.pos.user.id,
                            'action': 'add_new_line',
                            'time_action': new Date()
                        })
    ...
    models.Orderline = models.Orderline.extend({
            ...
            this.bind('remove', function () {
                ...
                if (this.pos.config.pos_order_tracking) {
                    this.order.tracking_ids.push({
                        'name': _t('User removed Product: ') + this.product.display_name,
                        'order_reference': this.order.uid,
        ...
        set_quantity: function (quantity, keep_price) {
            ...
            const self = this
            const res = _super_order_line.set_quantity.apply(this, arguments);
            ...
            if (!this.pos.the_first_load && this.pos.config.pos_order_tracking) {
                ...
                setTimeout(function () {
                    self.order.tracking_ids.push({
                        'name': self.product.display_name + _t(' with Quantity: ') + quantity,
                        'order_reference': self.order.uid,
                        'user_id': self.pos.user.id,
        ...
        set_discount: function (discount) {
            ...
            if (!this.pos.the_first_load && this.pos.config.pos_order_tracking) {
                ...
                this.order.tracking_ids.push({
                    'name': _t('Set Discount product: ') + this.product.display_name + _t(' with discount: ') + discount,
                    'order_reference': this.order.uid,
                    'user_id': this.pos.user.id,
                    'action': 'trigger_update_line',
                    'time_action': new Date()
