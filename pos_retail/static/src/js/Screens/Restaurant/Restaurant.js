odoo.define('pos_retail.restaurant', function (require) {
    ...
    const sync = require('pos_retail.synchronization');
    const multi_print = require('pos_restaurant.multiprint');
    ...
    models.PosModel = models.PosModel.extend({
        initialize: function (session, attributes) {
            ...
            restaurant_floor_model.loaded = function (self, floors) {
                ...
                if (floors.length) {
                    const floor_have_order_checkedin = {
                        id: 0,
                        name: _t('Waiting Check/Out'),
                        sequence: 0,
                        table_ids: [],
                        tables: [],
                        background_color: floors[0]['background_color']
                    }
        ...
        sync_new_order: function (vals) {
            ...
            if (order_exist) {
                console.error('order exist in pos')
                return order_exist
            } else {
        ...
        sync_request_printer: async function (vals) { // todo: - update variable set_dirty of line
            ...
            this.lastSelectedOrder = selectedOrder
            const computeChanges = vals.computeChanges
            if (order) {
                if (this.config.point_print) {
                    ...
                    for (let i = 0; i < printers.length; i++) {
                        ...
                        if (changes['new'].length > 0 || changes['cancelled'].length > 0) {
                            const newDatas = changes['new']
                            const cancelDatas = changes['cancelled']
                            changes['new'] = []
                            changes['cancelled'] = []
                            for (let n = 0; n < newDatas.length; n++) {
                                let data = changes
                                data['new'] = [newDatas[n]]
                                let qty_print = data['new'][0]['qty']
                            ...
                            for (let n = 0; n < cancelDatas.length; n++) {
                                let data = changes
                                data['cancelled'] = [cancelDatas[n]]
                                let qty_print = data['cancelled'][0]['qty']
                                for (let x = 0; x < qty_print; x++) {
                                    let orderReceipt = order.buildReceiptKitchen(data);
            ...
            // todo: - trigger for kitchen screen reload
            if (this.config.screen_type && (computeChanges.new.length > 0 || computeChanges.cancelled.length > 0)) {
                if (this.config.screen_type != 'kitchen') { // todo: - if waiters or cashiers, and order not still on Session, reject it
                    let orderExist = this.get_order_by_uid(computeChanges.uid)
                    if (!orderExist) {
                        return true;
                    }
                }
                posbus.trigger('newOrderReceiptsComing', computeChanges); // todo: - trigger kitchen screen
            }
        },

        async automatic_print_request_of_waiters() {
            let request_printers = this.db.load('request_printers', []);
            if (request_printers.length > 0) {
                for (let i = 0; i < request_printers.length; i++) {
                    let request = request_printers[i];
                    $('.pos-receipt-container').html(request['receiptHtml']);
                    window.print()
                    this.db.removeRequestPrintFromTicketNumber(request['ticket_number'])
                }
            }
            setTimeout(() => this.automatic_print_request_of_waiters(), 1000);
        },
