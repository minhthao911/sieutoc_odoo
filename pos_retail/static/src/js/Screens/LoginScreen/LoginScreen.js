odoo.define('pos_retail.LoginScreen', function (require) {
    ...
    const RetailLoginScreen = (LoginScreen) =>
        class extends LoginScreen {
            ...
            mounted() {
                super.mounted();
                if (this.env.pos.config.employee_automatic_login) {
                    const employee = this.env.pos.employees.find(emp => emp['user_id'] && emp['user_id'][0] == this.env.pos.user.id)
                    if (employee) {
                        this.env.pos.set_cashier(employee)
                        this.back()
                        this.env.pos.alert_message({
                            title: this.env._t("Login Successfully !"),
                            body: this.env._t('Welcome Back: ') + employee.name,
                        })
                        this.env.pos.config.employee_automatic_login = false
                    }
                }
            }
