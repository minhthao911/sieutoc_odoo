...
class pos_controller(PosController):
    ...
    def pos_web(self, config_id=False, **k):
        _logger.info('started pos controller')
        ...
        if (not pos_session and config_id) or k.get('switchPos'):
            ...
            pos_session = request.env['pos.session'].sudo().search(domain, limit=1)
            # ----------------------------------
            # TODO: for user change pos config via pos screen
            # ----------------------------------
            if not pos_session:
                try:
                    ...
                    _logger.error('===>>>> Error when open new session: %s' % ex)
        ...
        request.env['bus.bus']._sendone(
            pos_session.user_id.partner_id, 'pos.session.login', {
    ...
    def publicScanPlacedOrder(self, uid=None, **kk):
        ...
        for session in sessions:
            if session.user_id and session.user_id.partner_id:
                request.env['bus.bus']._sendone(
                    session.user_id.partner_id, 'pos.confirm.place.order', {
                        'uid': uid
                    })
    ...
    @http.route(['/pos/get/license'], type='http', auth='public')
    def getLicense(self, license='', **k):
        license_started_date = request.env['ir.config_parameter'].sudo().get_param('license_started_date')
        license = request.env['ir.config_parameter'].sudo().get_param('license')
        return json.dumps({
            'code': 200,
            'license': license if license else 'N/A',
            'license_started_date': license_started_date if license_started_date else 'N/A',
        })
