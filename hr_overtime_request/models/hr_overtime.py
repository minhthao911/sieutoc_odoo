...
class hr_overtime(models.Model):
    ...
    def unlink(self):
        for request in self:
            if request.state not in ('draft', 'cancel'):
                raise UserError(_('You cannot delete an overtime request which is not draft or cancelled.'))
    ...
    def onchange_employee_id(self):
        if self.employee_id.sudo().category_ids:
            self.category_id = self.employee_id.sudo().category_ids[0].id
    ...
    employee_id = fields.Many2one('hr.employee', string="Employee", index=True, required=True, default=_employee_get, states={'draft':[('readonly', False)]}, readonly=True)
    ...
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company, required=True, readonly=False, states={'validate':[('readonly', True)]})
    ...
    def onchange_start_date(self):
        if self.date_to and self.date_from:
            date_start = datetime.strptime(str(self.date_from), "%Y-%m-%d %H:%M:%S")
            date_end = datetime.strptime(str(self.date_to), "%Y-%m-%d %H:%M:%S")
